...
if __name__ == '__main__':
    ...
    client_1 = network_2.Host(1)
    object_L.append(client_1)

    client_2 = network_2.Host(2)
    object_L.append(client_2)

    server_1 = network_2.Host(3)
    object_L.append(server_1)

    server_2 = network_2.Host(4)
    object_L.append(server_2)


    router_a = network_2.Router(name='A', intf_count=4, max_queue_size=router_queue_size)
    router_b = network_2.Router(name='B', intf_count=2, max_queue_size=router_queue_size)
    router_c = network_2.Router(name='C', intf_count=2, max_queue_size=router_queue_size)
    ...
    link_layer.add_link(link_2.Link(client_1, 0, router_a, 0, 50))
    link_layer.add_link(link_2.Link(client_2, 1, router_a, 1, 50))

    # Links for Router A
    link_layer.add_link(link_2.Link(router_a, 2, router_b, 0, 50))
    link_layer.add_link(link_2.Link(router_a, 3, router_c, 0, 50))

    # Links for Router B
    link_layer.add_link(link_2.Link(router_b, 1, router_d, 0, 50))

    # Links for Router C
    link_layer.add_link(link_2.Link(router_c, 1, router_d, 1, 50))

    # Links for Router D
    link_layer.add_link(link_2.Link(router_d, 2, server_1, 0, 50))
    link_layer.add_link(link_2.Link(router_d, 3, server_2, 0, 50))
